/**
 * AICommandPalette - Notion-style inline AI command menu
 * Triggered by Cmd+J, BubbleMenu AI button, or /ai slash command
 */
    ...
    >
      ...
      <div className="ai-palette-input-row">
        ...
        <div className="ai-palette-scope-wrapper">
          <button
            className="ai-palette-scope-btn"
        ...
        {/* @ Button */}
        <button className="ai-palette-icon-btn" title="Mention">
          <span>@</span>
        </button>
                ...
                <div key={action.id} className="ai-palette-action-wrapper">
                  <button
                    ...
                    onMouseEnter={() => {
                      ...
                      if (action.submenu) setActiveSubmenu(action.id);
                    }}
                    onMouseLeave={() => {
                      if (!action.submenu) setHoveredAction(null);
                    }}
                  ...
                  {action.submenu && activeSubmenu === action.id && (
                    ...
                    >
                      {action.submenu.map((item, idx) => (
                        <button
                          key={item.id}
                          className="ai-palette-submenu-item"
                          onClick={() => handleActionClick(action, item.id)}
                        >
                          {item.label}
                          {idx === 0 && (
                            <span className="ai-palette-shortcut">↵</span>
                          )}
                        </button>
                      ))}
                    </div>
      ...
      <style>{`
        ...
        .ai-palette-input-row {
          display: flex;
          align-items: center;
          gap: 10px;
          padding: 12px 14px;
          border-bottom: 1px solid var(--color-border-subtle, #f1f1ef);
        }
        ...
        .ai-palette-input::placeholder {
          color: var(--color-text-ghost, #c7c7c5);
        }

        .ai-palette-scope-wrapper {
          position: relative;
        }

        .ai-palette-scope-btn {
          display: flex;
          align-items: center;
          gap: 6px;
          padding: 6px 10px;
        ...
        .ai-palette-scope-btn:hover {
          background: var(--color-bg-hover, #f1f1ef);
        }

        .ai-palette-scope-dropdown {
          position: absolute;
          top: calc(100% + 4px);
          right: 0;
          min-width: 160px;
        ...
        .ai-palette-scope-item:hover {
          background: var(--color-bg-hover, #f1f1ef);
        }

        .ai-palette-scope-item.active {
          color: var(--color-accent, #2eaadc);
        }
        ...
        .ai-palette-send-btn:disabled {
          opacity: 0.5;
          cursor: not-allowed;
        }

        .ai-palette-send-btn.active {
          background: var(--color-accent, #2eaadc);
          color: #fff;
        }

        .ai-palette-send-btn.active:hover {
          background: var(--color-accent-hover, #0077b5);
        }
        ...
        .ai-palette-action.hovered {
          background: var(--color-bg-hover, #f1f1ef);
        }

        .ai-palette-action-icon {
          width: 20px;
          text-align: center;
          font-size: 15px;
          color: var(--color-text-secondary, #6b6b6b);
        ...
        .ai-palette-action-label {
          flex: 1;
        }

        .ai-palette-action-arrow {
          color: var(--color-text-muted, #9b9a97);
        }
        ...
        /* Scrollbar */
        .ai-palette-actions::-webkit-scrollbar {
          width: 6px;
        }

        .ai-palette-actions::-webkit-scrollbar-track {
          background: transparent;
        }

        .ai-palette-actions::-webkit-scrollbar-thumb {
