...
const CAPABILITIES_WIDGETS: WidgetCapability[] = [
  ...
  },
  {
    ...
    kind: "widget",
    label: "Fetch",
    description: "Fetch external content into an artifact",
    icon: "Globe",
    category: "knowledge",
    surfaces: ["slash_menu", "command_palette"],
    requiresProject: true,
    ...
    prompt: {
      ...
      variables: [{ name: "source", type: "string", required: true }],
    },
    defaultModel: "openrouter/anthropic/claude-3-5-sonnet",
    order: 150,
  },
  {
    ...
    parameters: [
      {
        name: "diagramType",
        type: "enum",
        options: ["relationships", "timeline", "hierarchy", "flowchart"],
    ...
    prompt: {
      ...
      variables: [
        { name: "diagramType", type: "string", required: true },
        { name: "document", type: "document", required: false },
        { name: "selection", type: "selection", required: false },
      ],
    },
