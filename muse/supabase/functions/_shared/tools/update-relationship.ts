...
import { isCoreRelationshipType, isSignificantStrengthChange } from "./approval-config.ts";
 ...
 * Strength changes below threshold (weakening) are considered significant.
 */
async function updateRelationshipNeedsApproval({ updates, type }: UpdateRelationshipArgs): Promise<boolean> {
  ...
  if (isSignificantStrengthChange(updates.strength)) return true;

  // Core relationship types need approval for any update
  return isCoreRelationshipType(type);
}
